...
import sys
try:
    in1 = sys.argv[1] #in1=all_08-10_blast/180719Ded_D18-6932_1.allblast_paired
...
def dictitems(inputdict):
    ...
    for seqID in inputdictkeys:
        ...
        for item in subdictkeys:
            if item == 'besteVal' or item in subdictkeys == 'bestScore' or item in subdictkeys == 'longest':
...
fileout=in2+'_culled' #D18-1329_culled
## begin writing files
print '----'
print 'for file:', in1
print '----'
...
filedict_items = dictitems(filedict[0]) #excludes 'besteVal' key
print 'number of total items in filedict is:', filedict_items

# write all lines in filedict to folder '1_rnafileout'
lines1=['SeqID\ttRNA\teVal\tstart\tend\tbitScore']
# only write seqIDs that have been discarded
lines2=['SeqID(disc)\ttRNA\teVal\tstart\tend\tbitScore']
# write seqIDs with unique tRNA match to 1_rnafileout_nodupe
...
with open('all_11_culledfileout/'+fileout+suffix[0], 'w') as fo:
    fo.write('\n'.join(lines1))
with open('all_11_culledfileout/'+fileout+suffix[1], 'w') as fo:
   fo.write('\n'.join(lines2))
with open('all_11_culledfileout/'+fileout+suffix[2], 'w') as fo:
   fo.write('\n'.join(lines3))
